...
FINAL_FILES = ['Summary/'+sample+'_snake1.txt' for sample in samples]
...
foldersList= ["Summary",
				...
				"Step6_plotCoverage_result", "Step7_bamCoverage_result"
				]
...
rule plotCoverage:
	...
	run:
		...
		"{output.txt} --outCoverageMetrics {output.txt} --plotFileFormat pdf -p 3 --plotWidth 25 --plotHeight 7"),  #$bar_sequencing_depth  not used
		shell("plotCoverage -b {input} -o {output.pdf2} --labels {wildcards.sample} -T {wildcards.sample} --outRawCounts "
		"{output.txt2} --outCoverageMetrics {output.txt2} --plotFileFormat pdf -p 3 --plotWidth 25 --plotHeight 7")
...
rule bamCoverage:
	...
	run:
		shell("bamCoverage -b {input} -o {output.bedGraph} -of 'bedgraph' -p 4 --effectiveGenomeSize 29903 --normalizeUsing RPGC "),
		shell("bamCoverage -b {input} -o {output.bigwig} -of 'bigwig' --binSize 5 -p 4 --effectiveGenomeSize 29903 --normalizeUsing RPGC ")
