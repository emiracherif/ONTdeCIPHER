...
rule artic_minion_medaka:
	...
	shell:
		...
		" --read-file ../{input.artic_guppyplex_fastq} "+str(myParamDict['primers'])+" {wildcards.sample}"
...
rule snpEff:
	...
	shell:
		"snpEff ann -v "+str(myParamDict['reference_genome_snpEff'])+" {input.vcf} -s {output.html} -o vcf > {output.vcf}"
